...
def train_lightning_model(config):
    ...
    experiment_name = getattr(config.logging, 'experiment_name', 'parkinsons_prediction')
    mlflow.set_experiment(experiment_name)
    
    with mlflow.start_run(run_name=f"lightning_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
        ...
        if config.task.get("use_preprocessing", False):
            ...
            logger.info(f"Loaded preprocessed data from {config.data['preprocessed_train_data_path']}")

        train_data, val_data, test_data = create_patient_splits(
            data, 
            config.task.test_size, 
            config.task.val_size, 
            config.task.seed
        )
        ...
        run_dir = Path("checkpoints") / f"lightning_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        if trainer.checkpoint_callback.best_model_path:
            ...
            mlflow.log_metric("best_val_smape", trainer.checkpoint_callback.best_model_score.item())
        ...
        logger.info(f"View results with: mlflow ui --backend-store-uri ./mlruns")
