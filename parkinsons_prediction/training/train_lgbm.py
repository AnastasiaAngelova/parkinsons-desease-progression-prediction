...
def train_lgbm_model(config):
    ...
    with mlflow.start_run(run_name=f"lgbm_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
        ...
        if config.task.get("use_preprocessing", False):
            ...
            logger.info(f"Loaded preprocessed data from {config.data['preprocessed_data_path']}")
        ...
        try:
            onnx_model = convert_lightgbm(
                model.model,
                initial_types=[('input', onnxmltools.convert.common.data_types.FloatTensorType([1, len(features)]))],
                target_opset=11
            )
