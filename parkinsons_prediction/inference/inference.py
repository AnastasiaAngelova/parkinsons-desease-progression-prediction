...
def predict_with_lgbm_onnx(session, features):
    ...
    if len(outputs.shape) == 2 and outputs.shape[1] > 1:
        ...
        raise ValueError(f"Unexpected output shape from LightGBM model: {outputs.shape}")
...
def predict_with_nn_onnx(session, features):
    ...
    logger.info(f"NN final predictions range: {predictions.min()} - {predictions.max()}")
...
def create_submission_dataframe(test_data, predictions):
    ...
    test_data["prediction_id"] = (
        test_data["patient_id"].astype(str) + "_" +
        test_data["visit_month"].astype(str) + "_" +
        test_data["updrs"] + "_plus_" +
        test_data["delta_months"].astype(str) + "_months"
    )
...
def run_inference(cfg: DictConfig):
    ...
    if cfg.task.get("use_preprocessing", False):
        ...
        test_data = pd.read_csv(cfg.data.get("test_data_path", "data/test_processed.csv"))
        logger.info(f"Loaded preprocessed test data")
    ...
    if not nn_model_path.exists():
        ...
        raise FileNotFoundError(f"Neural Network ONNX model not found at {nn_model_path}")
    ...
    logger.info(f"LightGBM predictions range: {lgbm_predictions.min()} - {lgbm_predictions.max()}")
    
    logger.info("Making predictions with Neural Network model...")
    nn_predictions = predict_with_nn_onnx(nn_session, X_test)
    logger.info(f"Neural Network predictions range: {nn_predictions.min()} - {nn_predictions.max()}")
    
    ensemble_method = cfg.task.get("ensemble_method", "average")
    
    if ensemble_method == "average":
        logger.info("Averaging predictions from both models...")
        ensemble_predictions = np.round((lgbm_predictions + nn_predictions) / 2).astype(int)
    ...
    if cfg.task.get("save_individual_predictions", False):
        ...
        logger.info(f"Individual predictions saved:")
