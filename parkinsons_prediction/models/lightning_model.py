...
class ParkinsonsLightningModel(pl.LightningModule):
    ...
    def __init__(self, n_features, n_hidden=64, n_layers=2, lr=2e-3, 
                 dropout=0.1, weight_decay=0.01, warmup_steps=1000):
        super().__init__()
    ...
    def on_validation_epoch_end(self):
        ...
        smape_score = smape_metric(
            targets_denorm.float().numpy(),
            preds_denorm.float().numpy()
        )
        ...
        # Store for plotting
        avg_val_loss = torch.mean(torch.stack([x['val_loss'] for x in self.validation_step_outputs])).item()
    ...
    def create_and_log_plots(self):
        ...
        # Resample training losses to match validation epochs
        train_losses_epoch = []
        steps_per_epoch = len(self.train_losses) // len(self.val_losses) if len(self.val_losses) > 0 else 1
        ...
        plt.plot(epochs[:len(train_losses_epoch)], train_losses_epoch, 'b-', label='Training Loss', alpha=0.7)
